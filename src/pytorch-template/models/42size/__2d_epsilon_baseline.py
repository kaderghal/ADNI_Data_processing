...
root_path = '/home/kadergha/ADERGHAL/ADNI_workspace/results/ADNI1_coupe_des/F_42P_F10_MS1_MB05D/HIPP/3D/AD-NC/'
...
class Epsilon_2D_Net(nn.Module):
    def __init__(self):
        super(Epsilon_2D_Net, self).__init__()
        ...
        self.conv2 = nn.Conv2d(32, 64, kernel_size=7, stride=1, padding=0)
...
def main():
    ...
    model = Epsilon_2D_Net().to(device)
    ...
    for epoch in range(num_epochs):
        for i, (d1, d2, v, labels) in enumerate(train_loader):
            ...
            d1 = d1[:, 20:23,:,:].to(device, dtype=torch.float)
            # print("d1 size:", d1.size())
            ...
            if steps % print_every == 0:
                ...
                with torch.no_grad():
                    for i, (v_d1, v_d2, v_v, v_labels) in enumerate(valid_loader):
                        ...
                        v_d1 = v_d1[:, 20:23,:,:].to(device, dtype=torch.float)
