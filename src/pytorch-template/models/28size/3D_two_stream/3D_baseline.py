...
class TwoStream_Hipp_3D(nn.Module):
    def __init__(self):
        super(TwoStream_Hipp_3D, self).__init__()
...
def main():
    ...
    model = TwoStream_Hipp_3D().to(device)
    ...
    for epoch in range(num_epochs):
        for i, (d1, d2, v_meta_data, labels) in enumerate(train_loader):
            ...
            d1 = d1[:, 13:16:1,:, :].to(device, dtype=torch.float)
            d2 = d2[:, 13:16:1,:, :].to(device, dtype=torch.float)
            ...
            if steps % print_every == 0:
                ...
                with torch.no_grad():
                    for i, (v_d1, v_d2, v_v, v_labels) in enumerate(valid_loader):
                        ...
                        v_d1 = v_d1[:, 13:16:1,:,:].to(device, dtype=torch.float)
                        v_d2 = v_d2[:, 13:16:1,:,:].to(device, dtype=torch.float)


                        v_labels = v_labels.to(device)
                        v_outputs = model(v_d1, v_d2)
