...
class Epsilon_2D_Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(9, 32, kernel_size=4, stride=1, padding=1)
        self.bn1 = nn.BatchNorm2d(32)
        self.sewrapper1 = SEWrapper(32, ratio)
        self.relu = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv2d(32, 64, kernel_size=2, stride=1, padding=0)
        self.bn2 = nn.BatchNorm2d(64)
        self.sewrapper2 = SEWrapper(64, ratio)
...
def main():
    ...
    for epoch in range(num_epochs):
        for i, (d1, d2, v, labels) in enumerate(train_loader):
            ...
            d1 = d1[:, 9:18:1,:,:].to(device, dtype=torch.float)
            # print("d1 size:", d1.size())
            ...
            if steps % print_every == 0:
                ...
                with torch.no_grad():
                    for i, (v_d1, v_d2, v_v, v_labels) in enumerate(valid_loader):
                        ...
                        v_d1 = v_d1[:, 9:18:1, :,:].to(device, dtype=torch.float)
