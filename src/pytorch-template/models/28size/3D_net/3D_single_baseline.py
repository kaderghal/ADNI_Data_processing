...
root_path = '/data/ADERGHAL/ADNI_workspace/results/ADNI1_des/F_28P_F10_MS2_MB04D/HIPP/3D/AD-NC/'
...
class SingleStream_Hipp_3D(nn.Module):
    def __init__(self):
        super(SingleStream_Hipp_3D, self).__init__()
...
def main():
    ...
    for epoch in range(num_epochs):
        for i, (d1, d2, v_meta_data, labels) in enumerate(train_loader):
            ...
            print("outpu", torch.max(outputs.data, 1))
            # Track the accuracy
            total = labels.size(0)
